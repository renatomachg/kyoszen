...
export default function Vacancies() {
  ...
  useEffect(() => {
    supabase
      .from("vacantes")
      .select("id,titulo,empresa,ubicacion,contrato,badge,badge_class,categoria")
      .eq("activa", true)
      .order("id")
      .limit(4)
      .then(({ data }) => {
  ...
  // No mostrar nada si no hay vacantes activas (o mientras carga)
  if (!loaded || vacantes.length === 0) return null;
            ...
            >
              ...
              >
                <div className="flex items-center justify-between mb-3">
                  <span className="text-[10.5px] font-bold text-muted uppercase tracking-wide">{vac.categoria}</span>
                  <span className={`rounded-md px-2.5 py-0.5 text-[11px] font-bold ${vac.badge_class}`}>{vac.badge}</span>
                </div>
                ...
                <div className="text-sm font-extrabold leading-tight text-navy">{vac.titulo}</div>
                <div className="text-xs text-muted mt-1 flex items-center gap-1">
                  <svg width="11" height="11" viewBox="0 0 16 16" fill="none">
                    <path d="M8 1.5C5.51 1.5 3.5 3.51 3.5 6c0 3.75 4.5 8.5 4.5 8.5s4.5-4.75 4.5-8.5c0-2.49-2.01-4.5-4.5-4.5Z" stroke="currentColor" strokeWidth="1.4" />
                ...
                <div className="flex items-center justify-between pt-3 border-t border-border mt-3">
                  <span className="text-[11px] text-muted font-semibold">{vac.empresa}</span>
