...
/**
 * The default behavioral instructions for Kyo.
 * This block is editable from the admin panel and stored in Supabase kyo_config.
 * Dynamic data (company info, vacantes, cursos, FAQs) is always appended automatically.
 */
export const DEFAULT_INSTRUCCIONES = `# Personalidad
...
## Paso 2 — EXPERIENCIA
Pregunta cuantos anos de experiencia tiene en ese puesto.
Solo esta pregunta, nada mas.
...
/**
 * Builds the full system prompt for Kyo.
 * @param customInstrucciones - Optional override for the behavioral block (from Supabase or admin preview).
 *                              If not provided, DEFAULT_INSTRUCCIONES is used.
 */
export function buildSystemPrompt(customInstrucciones?: string): string {
  ...
  const instrucciones = customInstrucciones?.trim() || DEFAULT_INSTRUCCIONES;
