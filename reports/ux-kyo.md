...
**Fecha:** 2026-07-20
**Cambios analizados:** Sin commits de código nuevos en los últimos 2 días. El último cambio de código fue el commit b0bc7b7 (2026-07-13) — 7 días sin tocar el código. Archivos del asistente revisados en esta sesión: `src/lib/assistant/system-prompt.ts`, `src/lib/assistant/tools.ts`, `src/lib/assistant/knowledge.ts`, `src/app/api/assistant/chat/route.ts`, `src/components/assistant/useChat.ts`, `src/components/assistant/ChatWidget.tsx`.

---

## Cambios Recientes Detectados

**Sin commits de código en 7 días.** Los últimos 4 commits son solo actualizaciones de `reports/ux-kyo.md` y `reports/salud-sitio.md`. Todos los bugs listados abajo siguen presentes exactamente como ayer. Los contadores de días continúan incrementando.

**Nueva observación (detectada hoy):** El saludo inicial hardcodeado en `useChat.ts` (`INITIAL_GREETING`, línea 18) y la mención del mismo saludo en `DEFAULT_INSTRUCCIONES` (`system-prompt.ts`, línea 16) no están sincronizados. Si el admin edita el saludo en el panel de Kyo, el system prompt se actualiza desde Supabase, pero el usuario sigue viendo el saludo antiguo hardcodeado. Esto causa que Kyo diga "Ya saludé con X" pero el usuario haya visto Y.

---

## 🔴 BUGS CRÍTICOS — BLOQUEANTES

### BUG 1 — Kyo recomienda vacantes demo, no de Supabase (39.º día sin fix)
...
### BUG 55 — `kyo_mensaje` graba datos personales en analytics (39.º día — LFPDPPP)
...
### BUG 56 — Endpoint `archivar-video` sin verificación de sesión admin (28.º día sin fix)
...
### BUG 65 — URL de WhatsApp no es clickeable en burbujas de Kyo (23.º día)
...
### BUG 66 — `reset()` no limpia `sessionStorage` → sobreescribe logs de conversaciones (22.º día)
...
### BUG 91 — `INITIAL_GREETING` hardcodeado desincronizado del system prompt (NUEVO — 1.º día)
...
### BUG 78 — PropuestaEditor sin confirmación antes de enviar correo al cliente (11.º día sin fix)
...
### BUG 79 — `PostModal.save()` cierra el modal sin verificar si el fetch falló (11.º día sin fix)
...
### BUG 89 — `deletePost()` sin verificación de error — el modal cierra aunque el DELETE haya fallado (6.º día)
...
### BUG 90 — `togglePublicado()` sin `res.ok` — estado UI diverge de la BD si el PATCH falla (6.º día)
...
### BUG 86 — Tres funciones de movimiento sin `res.ok` — calendario avanza aunque el PUT haya fallado (7.º día)
...
### BUG 87 — Input de fecha activo mientras el move está en curso (7.º día)
...
### BUG 84 — `StoryboardEditor` y `GuiaTecnicaEditor` sin `onSaved()` tras guardar exitoso (8.º día)
...
### BUG 85 — `GuiaTecnicaEditor`: tabla de montaje con `key={i}` — reorders causan pérdidas de foco (8.º día)
...
### BUG 80 — Botón × del último cuadro/beat no se deshabilita → clic sin efecto visible (11.º día)
...
### BUG 81 — Cambios sin guardar se pierden silenciosamente al cancelar (11.º día)
...
### BUG 82 — Triple caption divergente sin sincronización entre versiones (11.º día)
...
### BUG 83 — Cuadros con solo `tipo` seleccionado se filtran silenciosamente al guardar (11.º día)
...
### BUG 57 — `subirADrive()` sin timeout — bloquea hasta 5 min si Google falla (28.º día)
...
### BUG 60 — `fetch(version.video_url)` sin timeout (27.º día)
...
### BUG 61 — Filtros de navegación en Kyo con acentos faltantes → filtros silenciosos (26.º día)
...
### BUG 76 — Input de Kyo sin `maxLength` — riesgo de costos de tokens (13.º día)
...
### BUG 77 — Sin focus trap en ChatWidget — usuarios de teclado escapan del widget (13.º día)
...
### BUG 72 — AplicarModal no cierra con tecla Escape (14.º día)
...
### BUG 73 — Acentos faltantes en UBICACION_OPTIONS de AplicarModal (14.º día)
...
### BUG 74 — "Si, todo en orden" sin acento (14.º día)
...
### BUG 75 — AplicarModal fetch sin timeout — candidato bloqueado en "Enviando..." (14.º día)
...
### BUG 69 — "aqui" sin acento en DOS archivos (15.º día)
...
### BUG 70 — Error de red muestra string técnico al usuario (15.º día)
...
### BUG 71 — Widget de Kyo no cierra con tecla Escape (15.º día)
...
### BUG 68 — Hero muestra "10+ Años exp." vs knowledge base "3+" (16.º día)
**Archivos:** `src/components/sections/Hero.tsx` líneas 107, 157; `src/lib/assistant/knowledge.ts` líneas 78-79

**Confirmado presente hoy:** `knowledge.ts` línea 79 dice `"Años en el mercado": "3+"` pero Hero.tsx muestra `"10+"`. Inconsistencia visible en la landing.

**Fix:** Unificar con los datos reales en ambos archivos: "3+ Años" y "687+ candidatos".

---

### BUG 67 — Placeholder Hero sin acento: `"¿Que puesto buscas?"` (16.º día)
...
### BUG 62 — `Hero.tsx` usa `next/image` — viola regla de CLAUDE.md (25.º día)
...
### BUG 64 — Filtro "Marca" en `/vacantes` usa nombres demo ficticios (25.º día)
...
### BUG 26 — Markdown asteriscos aparecen literales en burbujas de Kyo (39.º día sin fix)
...
### BUG 44 — Inconsistencia usted/tú en system prompt (33.º día)
...
### BUG 54 — "Mas de $20k" sin acento en filtro de salario (39.º día)
...
### BUG 47 — `search_jobs` no incluye `salario_nota` (39.º día)
...
### BUG 48 — Sin ARIA live region en el chat widget (39.º día)
...
### BUG 51 — Fallback `"MXN bruto"` en vacantes sin `salario_nota` (39.º día)
...
### BUG 52 — Pérdida de contexto de perfil en conversaciones largas (39.º día)
...
### BUG 58 — `rutaDeStorage` falla silenciosamente → video no se borra de Storage (28.º día)
...
### BUG 59 — `StoryboardView` no renderiza el estado de video archivado (28.º día)
...
### BUG 16 — Memory leak en `rateLimitMap` (39.º día)
...
### BUG 50 — `generateStaticParams()` usa `JOBS` hardcoded (39.º día)
...
|---|-----------|----------|---------|------|
| 1 | BUG 1 — Vacantes reales en Kyo (Supabase) | Alto | Crítico | 39 |
| 2 | BUG 56 — Endpoint archivar-video sin auth | Bajo (15 min) | Crítico | 28 |
| 3 | BUG 55 — kyo_mensaje graba datos personales (LFPDPPP) | Bajo (1 línea) | Crítico | 39 |
| 4 | BUG 91 — INITIAL_GREETING desincronizado del system prompt | Bajo (10 min) | Alto | 1 |
| 5 | BUG 78 — PropuestaEditor sin confirm antes de notificar | Bajo (10 min) | Crítico | 11 |
| 6 | BUG 89 — deletePost() sin res.ok | Bajo (10 min) | Alto | 6 |
| 7 | BUG 90 — togglePublicado() sin res.ok | Bajo (10 min) | Alto | 6 |
| 8 | BUG 86 — moverPost* sin res.ok (3 funciones) | Bajo (15 min) | Alto | 7 |
| 9 | BUG 87 — Input fecha activo mientras moving=true | Bajo (1 atrib) | Bajo | 7 |
| 10 | BUG 84 — StoryboardEditor/GuiaTecnicaEditor sin onSaved() | Bajo (10 min) | Alto | 8 |
| 11 | BUG 79 — PostModal.save() no verifica res.ok | Bajo (15 min) | Alto | 11 |
| 12 | BUG 85 — key={i} en beats de montaje | Bajo (10 min) | Alto | 8 |
| 13 | BUG 80 — Botón × sin disabled en último cuadro/beat | Bajo (5 min) | Medio | 11 |
| 14 | BUG 81 — Cambios sin guardar se pierden silenciosamente | Bajo (20 min) | Alto | 11 |
| 15 | BUG 82 — Triple caption divergente sin sincronización | Bajo (30 min) | Alto | 11 |
| 16 | BUG 83 — Cuadros con solo tipo se filtran al guardar | Bajo (10 min) | Medio | 11 |
| 17 | BUG 65 — URL WhatsApp no clickeable en Kyo | Bajo (10 min) | Crítico | 23 |
| 18 | BUG 66 — reset() no limpia sessionStorage | Bajo (1 línea) | Crítico | 22 |
| 19 | BUG 62 — Hero usa next/image | Bajo (5 min) | Alto | 25 |
| 20 | BUG 68 — Hero "10+ años" vs knowledge "3+ años" | Bajo (1 min) | Alto | 16 |
| 21 | BUG 64 — Filtro Marca con nombres demo ficticios | Bajo (30 min) | Alto | 25 |
| 22 | BUG 61 — Filtros ubicación con acentos faltantes | Bajo (2 min) | Alto | 26 |
| 23 | BUG 57 — subirADrive sin timeout | Bajo (5 min) | Alto | 28 |
| 24 | BUG 60 — fetch(video_url) sin timeout | Bajo (1 línea) | Alto | 27 |
| 25 | Variables GOOGLE_* en VPS (no es código) | Bajo (5 min) | Alto | 29 |
| 26 | BUG 67 — Placeholder Hero sin acento "¿Que puesto?" | Bajo (1 char) | Alto | 16 |
| 27 | BUG 53 / BUG 73 / BUG 74 — Acentos en AplicarModal | Bajo (5 min) | Alto | 39/14/14 |
| 28 | BUG 26 — Markdown asteriscos en Kyo | Bajo (10 min) | Alto | 39 |
| 29 | BUG 69 — "aqui" sin acento en DOS archivos | Bajo (2 min) | Alto | 15 |
| 30 | BUG 70 — Error de red muestra string técnico al usuario | Bajo (5 min) | Alto | 15 |
| 31 | BUG 72 — AplicarModal no cierra con Escape | Bajo (5 min) | Alto | 14 |
| 32 | BUG 75 — AplicarModal fetch sin timeout | Bajo (10 min) | Alto | 14 |
| 33 | BUG 76 — Input Kyo sin maxLength | Bajo (10 min) | Alto | 13 |
| 34 | BUG 77 — Sin focus trap en ChatWidget | Bajo (20 min) | Medio | 13 |
| 35 | BUG 71 — Widget no cierra con Escape | Bajo (5 min) | Medio | 15 |
| 36 | BUG 44 — usted/tú inconsistente en prompt | Bajo (15 min) | Alto | 33 |
| 37 | BUG 51 — Fallback "MXN bruto" en vacantes | Bajo (1 min) | Alto | 39 |
| 38 | "Nueva conversacion" sin acento en ChatWidget | Bajo (1 min) | Medio | — |
| 39 | BUG 54 — "Mas de $20k" sin acento | Bajo (1 min) | Medio | 39 |
| 40 | BUG 59 — StoryboardView no renderiza estado archivado | Bajo (20 min) | Medio | 28 |
| 41 | Avisar a candidato que puede seguir chateando tras navegación | Bajo (10 min) | Alto | — |
| 42 | BUG 47 — salario_nota faltante en search_jobs | Bajo (20 min) | Medio | 39 |
| 43 | BUG 58 — rutaDeStorage falla silenciosamente | Bajo (5 min) | Medio | 28 |
| 44 | BUG 48 — Sin ARIA live region en chat widget | Bajo (1 línea) | Medio | 39 |
| 45 | BUG 52 — Pérdida de perfil en conversaciones largas | Bajo (1 línea) | Medio | 39 |
...
| 57 | BUG 16 — Memory leak rateLimitMap | Bajo (5 min) | Medio | 39 |
| 58 | BUG 50 — generateStaticParams usa JOBS hardcoded | Medio | Bajo | 39 |
