...
**Fecha:** 2026-07-03
**Cambios analizados:** Sin commits de código desde 2026-06-22 (12.º día consecutivo sin fix).
...
Ningún commit de código desde el 2026-06-22 (12 días). Los únicos commits son reportes automáticos (`ux-kyo.md`, `tendencias-julio-2026.md`, `dependencias.md`).

**Pendiente operativo crítico (configuración, no código):** Las 4 variables `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `GOOGLE_REFRESH_TOKEN` y `GOOGLE_DRIVE_FOLDER_ID` siguen sin estar en el `ecosystem.config.js` del VPS. El botón "🗄️ Liberar espacio" devuelve 503 en producción (12.º día pendiente).

> ⚠️ **Alerta de acumulación:** 23 bugs abiertos con antigüedad de 22+ días. Los 5 más críticos (BUG 1, 55, 56, 65, 66) tienen impacto directo en candidatos en producción ahora mismo. Todos los bugs del reporte anterior siguen sin corrección.

---

## 🔴 BUGS CRÍTICOS — BLOQUEANTES

### BUG 1 — Kyo recomienda vacantes demo, no de Supabase (22.º día sin fix)
...
### BUG 55 — `kyo_mensaje` graba datos personales en analytics (22.º día — LFPDPPP)
...
### BUG 56 — Endpoint `archivar-video` sin verificación de sesión admin (11.º día sin fix)
...
### BUG 65 — URL de WhatsApp no es clickeable en burbujas de Kyo (6.º día)
...
### BUG 66 — `reset()` no limpia `sessionStorage` → sobreescribe logs de conversaciones (5.º día)
...
### BUG 57 — `subirADrive()` sin timeout — bloquea hasta 5 min si Google falla (11.º día)
...
### BUG 60 — `fetch(version.video_url)` sin timeout (10.º día)
...
### BUG 61 — Filtros de navegación en Kyo con acentos faltantes → filtros silenciosos (9.º día)
...
### BUG 62 — `Hero.tsx` usa `next/image` — viola regla de CLAUDE.md (8.º día)
...
### BUG 63 — Estadística Hero "+7000 colocados" vs knowledge.ts "687+" — contradicción visible (8.º día)
...
### BUG 64 — Filtro "Marca" en `/vacantes` usa nombres demo ficticios (8.º día)
...
### BUG 58 — `rutaDeStorage` falla silenciosamente → video no se borra de Storage (11.º día)
...
### BUG 59 — `StoryboardView` no renderiza el estado de video archivado (11.º día)
...
### BUG 26 — Markdown asteriscos aparecen literales en burbujas de Kyo (22.º día sin fix)
...
### BUG 46 — "aqui" sin acento en el saludo inicial de Kyo (22.º día)
...
### BUG 53 — 6 cadenas sin acento en AplicarModal (22.º día)
...
### BUG 44 — Inconsistencia usted/tú en system prompt de Kyo (16.º día)
...
### BUG 54 — "Mas de $20k" sin acento en filtro de salario (22.º día)
...
### BUG 47 — `search_jobs` no incluye `salario_nota` (22.º día)
...
### BUG 48 — Sin ARIA live region en el chat widget (22.º día)
...
### BUG 51 — Fallback `"MXN bruto"` en vacantes sin `salario_nota` (22.º día)
...
### BUG 52 — Pérdida de contexto de perfil en conversaciones largas (22.º día)
...
### BUG 50 — `generateStaticParams()` usa `JOBS` hardcoded (22.º día)
...
### BUG 16 — Memory leak en `rateLimitMap` (22.º día)
...
|---|-----------|----------|---------|------|
| 1 | BUG 1 — Vacantes reales en Kyo (Supabase) | Alto | Crítico | 22 |
| 2 | BUG 56 — Endpoint archivar-video sin auth | Bajo (15 min) | Crítico | 11 |
| 3 | BUG 65 — URL WhatsApp no clickeable en Kyo | Bajo (10 min) | Crítico | 6 |
| 4 | BUG 66 — reset() no limpia sessionStorage | Bajo (1 línea) | Crítico | 5 |
| 5 | BUG 55 — kyo_mensaje graba datos personales (LFPDPPP) | Bajo (1 línea) | Crítico | 22 |
| 6 | BUG 62 — Hero usa next/image | Bajo (5 min) | Alto | 8 |
| 7 | BUG 63 — Estadísticas +7000 vs 687+ contradicción | Bajo (1 min) | Alto | 8 |
| 8 | BUG 64 — Filtro Marca con nombres demo ficticios | Bajo (30 min) | Alto | 8 |
| 9 | BUG 61 — Filtros ubicación con acentos faltantes | Bajo (1 min) | Alto | 9 |
| 10 | BUG 57 — `subirADrive` sin timeout | Bajo (5 min) | Alto | 11 |
| 11 | BUG 60 — `fetch(video_url)` sin timeout | Bajo (1 línea) | Alto | 10 |
| 12 | Variables `GOOGLE_*` en VPS (no es código) | Bajo (5 min) | Alto | 12 |
| 13 | BUG 53 — 6 acentos faltantes en AplicarModal | Bajo (5 min) | Alto | 22 |
| 14 | BUG 26 — Markdown asteriscos en Kyo | Bajo (10 min) | Alto | 22 |
| 15 | BUG 46 — Acento "aquí" en saludo inicial Kyo | Bajo (1 min) | Alto | 22 |
| 16 | BUG 44 — Registro usted/tú inconsistente en prompt | Bajo (15 min) | Alto | 16 |
| 17 | BUG 51 — Fallback "MXN bruto" en vacantes | Bajo (1 min) | Alto | 22 |
| 18 | "Nueva conversacion" sin acento en ChatWidget | Bajo (1 min) | Medio | — |
| 19 | BUG 54 — "Mas de $20k" sin acento | Bajo (1 min) | Medio | 22 |
| 20 | BUG 59 — StoryboardView no renderiza estado archivado | Bajo (20 min) | Medio | 11 |
| 21 | Avisar a candidato que puede seguir chateando tras navegación | Bajo (10 min) | Alto | — |
| 22 | BUG 47 — salario_nota faltante en search_jobs | Bajo (20 min) | Medio | 22 |
| 23 | BUG 58 — rutaDeStorage falla silenciosamente | Bajo (5 min) | Medio | 11 |
| 24 | BUG 48 — Sin ARIA live region en chat widget | Bajo (1 línea) | Medio | 22 |
| 25 | BUG 52 — Pérdida de perfil en conversaciones largas | Bajo (1 línea) | Medio | 22 |
...
| 36 | BUG 16 — Memory leak rateLimitMap | Bajo (5 min) | Medio | 22 |
| 37 | BUG 50 — generateStaticParams usa JOBS hardcoded | Medio | Bajo | 22 |
