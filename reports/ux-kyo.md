...
**Fecha:** 2026-07-15
**Cambios analizados:** Sin commits nuevos hoy. Último cambio relevante: `src/app/admin/(panel)/redes-sociales/page.tsx` (commit b0bc7b7, 2026-07-14). Archivos del asistente revisados: `src/lib/assistant/system-prompt.ts`, `src/lib/assistant/tools.ts`, `src/lib/assistant/knowledge.ts`, `src/app/api/assistant/chat/route.ts`.

---

## Cambios Recientes Detectados

**Sin commits nuevos hoy.** El último cambio fue el commit b0bc7b7 (2026-07-14): botón "📅 Mover" en el modal de detalle de una publicación — permite mover a cualquier fecha (incluso de meses lejanos) sin arrastrar mes a mes. La función `moverAFecha()` (líneas 866-888 de `redes-sociales/page.tsx`) fue correctamente implementada con `res.ok`, `try/catch/finally`, alert de error y navegación automática con `seguirFecha()`. Los 3 bugs de ayer (86-88) siguen sin corregir en las funciones hermanas de drag-and-drop.

**2 bugs nuevos encontrados hoy** (`deletePost` y `togglePublicado`) en la misma revisión del archivo.

---

## 🔴 BUGS CRÍTICOS — BLOQUEANTES

### BUG 1 — Kyo recomienda vacantes demo, no de Supabase (34.º día sin fix)
...
### BUG 55 — `kyo_mensaje` graba datos personales en analytics (34.º día — LFPDPPP)
...
### BUG 56 — Endpoint `archivar-video` sin verificación de sesión admin (23.º día sin fix)
...
### BUG 65 — URL de WhatsApp no es clickeable en burbujas de Kyo (18.º día)
...
### BUG 66 — `reset()` no limpia `sessionStorage` → sobreescribe logs de conversaciones (17.º día)
...
### BUG 78 — PropuestaEditor sin confirmación antes de enviar correo al cliente (6.º día sin fix)
...
### BUG 79 — `PostModal.save()` cierra el modal sin verificar si el fetch falló (6.º día sin fix)
...
## 🟠 BUGS NUEVOS (detectados hoy, 2026-07-15)

### BUG 89 — `deletePost()` sin verificación de error — el modal cierra aunque el DELETE haya fallado (1.º día)
...
Si el DELETE falla (sesión expirada, error de red), el admin ve el modal cerrar, el calendario se refresca y la publicación sigue ahí — sin mensaje de error.
...
### BUG 90 — `togglePublicado()` sin `res.ok` — estado UI diverge de la BD si el PATCH falla (1.º día)
...
Si el PATCH falla, el botón muestra "● Visible para el cliente" pero en la BD el post sigue siendo borrador (o viceversa). La confusión solo se descubre al recargar.
...
## 🟠 BUGS PENDIENTES DE DÍAS ANTERIORES

### BUG 86 — `moverPostPeriodo()` sin `res.ok` — movimiento por flechas ‹ › sin detección de error (2.º día)
**Archivo:** `src/app/admin/(panel)/redes-sociales/page.tsx` — función `moverPostPeriodo` líneas 1309-1312
...
El admin arrastra a la flecha ›, el calendario salta al mes siguiente, pero la publicación sigue en el mes anterior en la BD. No hay aviso de error.
...
### BUG 87 — `intercambiarFechas()` sin rollback — estado inconsistente si un fetch falla (2.º día)
...
### BUG 88 — Input de fecha activo mientras el move está en curso (2.º día)
**Archivo:** `src/app/admin/(panel)/redes-sociales/page.tsx` — `PostDetail` línea 929

Cuando `moving === true`, el botón "✓ Mover" se deshabilita y muestra "..." (correcto), pero el `<input type="date">` sigue activo. El admin puede cambiar la fecha mientras la operación está en curso.
...
### BUG 84 — `StoryboardEditor` y `GuiaTecnicaEditor` sin `onSaved()` tras guardar exitoso (3.º día)
...
### BUG 85 — `GuiaTecnicaEditor`: tabla de montaje con `key={i}` — reorders causan pérdidas de foco (3.º día)
...
### BUG 80 — Botón × del último cuadro/beat no se deshabilita → clic sin efecto visible (6.º día)
...
### BUG 81 — Cambios sin guardar se pierden silenciosamente al cancelar (6.º día)
...
### BUG 82 — Triple caption divergente sin sincronización entre versiones (6.º día)
...
### BUG 83 — Cuadros con solo `tipo` seleccionado se filtran silenciosamente al guardar (6.º día)
...
### BUG 57 — `subirADrive()` sin timeout — bloquea hasta 5 min si Google falla (23.º día)
...
### BUG 60 — `fetch(version.video_url)` sin timeout (22.º día)
...
### BUG 61 — Filtros de navegación en Kyo con acentos faltantes → filtros silenciosos (21.º día)
...
### BUG 76 — Input de Kyo sin `maxLength` — riesgo de costos de tokens (8.º día)
...
### BUG 77 — Sin focus trap en ChatWidget — usuarios de teclado escapan del widget (8.º día)
...
### BUG 72 — AplicarModal no cierra con tecla Escape (9.º día)
...
### BUG 73 — Acentos faltantes en UBICACION_OPTIONS de AplicarModal (9.º día)
...
### BUG 74 — "Si, todo en orden" sin acento (9.º día)
...
### BUG 75 — AplicarModal fetch sin timeout — candidato bloqueado en "Enviando..." (9.º día)
...
### BUG 69 — "aqui" sin acento en DOS archivos (10.º día)
...
### BUG 70 — Error de red muestra string técnico al usuario (10.º día)
...
### BUG 71 — Widget de Kyo no cierra con tecla Escape (10.º día)
...
### BUG 68 — Hero muestra "10+ Años exp." vs knowledge base "3+" (11.º día)
...
### BUG 67 — Placeholder Hero sin acento: `"¿Que puesto buscas?"` (11.º día)
...
### BUG 62 — `Hero.tsx` usa `next/image` — viola regla de CLAUDE.md (20.º día)
...
### BUG 64 — Filtro "Marca" en `/vacantes` usa nombres demo ficticios (20.º día)
...
### BUG 26 — Markdown asteriscos aparecen literales en burbujas de Kyo (34.º día sin fix)
...
### BUG 44 — Inconsistencia usted/tú en system prompt (28.º día)
...
### BUG 54 — "Mas de $20k" sin acento en filtro de salario (34.º día)
...
### BUG 47 — `search_jobs` no incluye `salario_nota` (34.º día)
...
### BUG 48 — Sin ARIA live region en el chat widget (34.º día)
...
### BUG 51 — Fallback `"MXN bruto"` en vacantes sin `salario_nota` (34.º día)
...
### BUG 52 — Pérdida de contexto de perfil en conversaciones largas (34.º día)
...
### BUG 58 — `rutaDeStorage` falla silenciosamente → video no se borra de Storage (23.º día)
...
### BUG 59 — `StoryboardView` no renderiza el estado de video archivado (23.º día)
...
### BUG 16 — Memory leak en `rateLimitMap` (34.º día)
...
### BUG 50 — `generateStaticParams()` usa `JOBS` hardcoded (34.º día)
...
|---|-----------|----------|---------|------|
| 1 | BUG 1 — Vacantes reales en Kyo (Supabase) | Alto | Crítico | 34 |
| 2 | BUG 56 — Endpoint archivar-video sin auth | Bajo (15 min) | Crítico | 23 |
| 3 | BUG 78 — PropuestaEditor sin confirm antes de notificar | Bajo (10 min) | Crítico | 6 |
| 4 | BUG 89 — deletePost() sin res.ok | Bajo (10 min) | Alto | 1 (nuevo) |
| 5 | BUG 90 — togglePublicado() sin res.ok | Bajo (10 min) | Alto | 1 (nuevo) |
| 6 | BUG 86 — moverPostPeriodo() sin res.ok en drag flechas | Bajo (15 min) | Alto | 2 |
| 7 | BUG 87 — intercambiarFechas() sin rollback | Bajo (15 min) | Alto | 2 |
| 8 | BUG 88 — Input fecha activo mientras moving=true | Bajo (1 atrib) | Bajo | 2 |
| 9 | BUG 84 — StoryboardEditor/GuiaTecnicaEditor sin onSaved() | Bajo (10 min) | Alto | 3 |
| 10 | BUG 79 — PostModal.save() no verifica res.ok | Bajo (15 min) | Alto | 6 |
| 11 | BUG 85 — key={i} en beats de montaje | Bajo (10 min) | Alto | 3 |
| 12 | BUG 80 — Botón × sin disabled en último cuadro/beat | Bajo (5 min) | Medio | 6 |
| 13 | BUG 81 — Cambios sin guardar se pierden silenciosamente | Bajo (20 min) | Alto | 6 |
| 14 | BUG 82 — Triple caption divergente sin sincronización | Bajo (30 min) | Alto | 6 |
| 15 | BUG 83 — Cuadros con solo tipo se filtran al guardar | Bajo (10 min) | Medio | 6 |
| 16 | BUG 65 — URL WhatsApp no clickeable en Kyo | Bajo (10 min) | Crítico | 18 |
| 17 | BUG 66 — reset() no limpia sessionStorage | Bajo (1 línea) | Crítico | 17 |
| 18 | BUG 55 — kyo_mensaje graba datos personales (LFPDPPP) | Bajo (1 línea) | Crítico | 34 |
| 19 | BUG 62 — Hero usa next/image | Bajo (5 min) | Alto | 20 |
| 20 | BUG 68 — Hero "10+ años" vs knowledge "3+ años" | Bajo (1 min) | Alto | 11 |
| 21 | BUG 64 — Filtro Marca con nombres demo ficticios | Bajo (30 min) | Alto | 20 |
| 22 | BUG 61 — Filtros ubicación con acentos faltantes | Bajo (2 min) | Alto | 21 |
| 23 | BUG 57 — subirADrive sin timeout | Bajo (5 min) | Alto | 23 |
| 24 | BUG 60 — fetch(video_url) sin timeout | Bajo (1 línea) | Alto | 22 |
| 25 | Variables GOOGLE_* en VPS (no es código) | Bajo (5 min) | Alto | 24 |
| 26 | BUG 67 — Placeholder Hero sin acento "¿Que puesto?" | Bajo (1 char) | Alto | 11 |
| 27 | BUG 53 / BUG 73 / BUG 74 — Acentos faltantes en AplicarModal | Bajo (5 min) | Alto | 34/9/9 |
| 28 | BUG 26 — Markdown asteriscos en Kyo | Bajo (10 min) | Alto | 34 |
| 29 | BUG 69 — "aqui" sin acento en DOS archivos | Bajo (2 min) | Alto | 10 |
| 30 | BUG 70 — Error de red muestra string técnico al usuario | Bajo (5 min) | Alto | 10 |
| 31 | BUG 72 — AplicarModal no cierra con Escape | Bajo (5 min) | Alto | 9 |
| 32 | BUG 75 — AplicarModal fetch sin timeout | Bajo (10 min) | Alto | 9 |
| 33 | BUG 76 — Input Kyo sin maxLength | Bajo (10 min) | Alto | 8 |
| 34 | BUG 77 — Sin focus trap en ChatWidget | Bajo (20 min) | Medio | 8 |
| 35 | BUG 71 — Widget no cierra con Escape | Bajo (5 min) | Medio | 10 |
| 36 | BUG 44 — usted/tú inconsistente en prompt | Bajo (15 min) | Alto | 28 |
| 37 | BUG 51 — Fallback "MXN bruto" en vacantes | Bajo (1 min) | Alto | 34 |
| 38 | "Nueva conversacion" sin acento en ChatWidget | Bajo (1 min) | Medio | — |
| 39 | BUG 54 — "Mas de $20k" sin acento | Bajo (1 min) | Medio | 34 |
| 40 | BUG 59 — StoryboardView no renderiza estado archivado | Bajo (20 min) | Medio | 23 |
| 41 | Avisar a candidato que puede seguir chateando tras navegación | Bajo (10 min) | Alto | — |
| 42 | BUG 47 — salario_nota faltante en search_jobs | Bajo (20 min) | Medio | 34 |
| 43 | BUG 58 — rutaDeStorage falla silenciosamente | Bajo (5 min) | Medio | 23 |
| 44 | BUG 48 — Sin ARIA live region en chat widget | Bajo (1 línea) | Medio | 34 |
| 45 | BUG 52 — Pérdida de perfil en conversaciones largas | Bajo (1 línea) | Medio | 34 |
...
| 57 | BUG 16 — Memory leak rateLimitMap | Bajo (5 min) | Medio | 34 |
| 58 | BUG 50 — generateStaticParams usa JOBS hardcoded | Medio | Bajo | 34 |
