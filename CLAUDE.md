...
- **Comentarios con minuto, sin migración:** el minuto va al frente del texto (`[1:23] el logo se ve chico`). `conMarcaTiempo` / `leerMarcaTiempo` / `formatearTiempo` (m:ss y h:mm:ss). Al ponerse a escribir con el video avanzado, se pausa y el minuto se toma solo; también hay "Marcar el minuto actual" y "×" para quitarlo. En el hilo, el minuto es un botón (`ChipMinuto` / `ChipMinutoAdmin`) que lleva el reproductor ahí. Así el minuto viaja solo en los correos.
- **Cliente:** `VideoCompleto` en `ProyectosCliente.tsx` (reproductor + hilo + caja con minuto). **Pedir cambios ya no obliga a repetir** si hay comentarios (el POST del comentario se omite y solo cambia el estado). Se oculta "Aprobar todas las pendientes" en etapas de entregable único. El aviso por correo dice "comentó en el video completo".
- **Nginx del VPS:** se subieron `proxy_read_timeout`/`proxy_send_timeout` a 600s en el sitio kyoszen. Estaba en 60s por defecto y un video completo tarda más en comprimirse con ffmpeg (1 CPU) → la carga moría con 504 aunque el video sí se procesara. Respaldo en `/root/nginx-kyoszen.bak.20260912-195622`. **OJO:** nunca dejar respaldos dentro de `sites-enabled/` (nginx carga todo lo que hay ahí), y `sites-enabled/kyoszen` es un symlink a `sites-available/kyoszen`: editar el de `sites-available`, porque `sed -i` sobre el symlink lo convierte en archivo normal.
- **Límite:** Storage acepta 50 MB por archivo (tope del plan) después de comprimir (1080p, crf 28). Unos minutos caben; un video largo podría no caber.
...
- **UI** (`src/components/admin/ImportarCampana.tsx`): un solo panel con textarea + zona de arrastre (imágenes y documentos) + botones de subida. Las capturas se reescalan a 1600px y se mandan como JPEG base64 (evita cuerpos gigantes). Vista previa con los datos leídos de la campaña, casillas por anuncio y **selector de destino: campaña nueva o agregar a una existente**.
- **`+ Agregar anuncio`** dentro de una campaña abierta (`POST /api/admin/campanas/[id]/anuncios`) para sumar un puesto a mano sin importar nada.
- **Eliminar:** ícono de bote en la fila de la campaña (borra en cascada anuncios + comentarios) y botón "Eliminar anuncio" en cada anuncio. El DELETE de anuncio recalcula el rollup de la campaña. Borrar aquí **no toca nada en Meta**.
- **`src/components/ui/ConfirmModal.tsx`** — modal de confirmación con la identidad del panel (cabecera navy o roja, lista de lo afectado, caja de aviso, nota al pie). Reemplaza `confirm()`/`prompt()`/`alert()` nativos en Campañas: publicar (avisa cuántos anuncios van sin arte), eliminar campaña (**pide escribir ELIMINAR si está publicada**), eliminar anuncio y agregar anuncio (modal con un campo). Los errores salen en un banner rojo arriba, no en `alert()`. **Reusable: úsalo en el resto del admin en vez de diálogos nativos.** Se monta solo cuando está abierto (evita `set-state-in-effect` de React 19).
- Todo nace en **borrador**. La barrera con el cliente sigue siendo el flag `publicado`.
- **Meta:** el conector existe y la cuenta `act_1657503055383140` ("Kyoszen") es legible por API — las 3 campañas están creadas pero **en pausa y sin conjuntos ni anuncios**; los 3 formularios sí existen completos en la página `1116560118200266`. El usuario lleva la parte de Meta por su lado; **no tocar la cuenta de anuncios**. Un botón "Traer de Meta" queda como fase 2 (requiere app de Meta + token de usuario de sistema en el VPS).
...
- **"todo Kyoszen" — HECHO:** barrido corporativo de TODO el admin en 3 tandas (Redes Sociales, Analytics, Estratega, CRM, Blog, Correos, Vacantes, SEO, Cursos, Kyo, Cuestionarios, Contenido). Auditoría: 0 emojis decorativos en las 17 secciones. Componente compartido `src/components/ui/IconUI.tsx` (íconos SVG de línea reutilizables, set ampliable). Cambio siempre quirúrgico (solo emojis→íconos/puntos, sin tocar lógica). Se conserva navy + amarillo `#FFCC00`. Las secciones ya limpias (Dashboard, Contactos, Aplicaciones, Testimonios) no requirieron cambios. Nota: si aparece una sección nueva, usar `IconUI` y no meter emojis decorativos.
